...
(function () {
    ...
    PStrScan = PClass.derive(Object.defineProperties({
        ...
        unscan: function () {
            if (this.match) {
                this.captures = this.match = null;
                this.pos = this.last;
                this.last =this._before && this._before.shift()||0;
            }
            else {
                throw new Error(
        ...
        revert : function ( val ) {
            if(this._before){
                this.captures = this.match = null;
                this.pos = val || this.last; this.last = val? 0 : this._before.pop()||0;
            }
    ...
    //-----------------------------------------------------------------------
    {
        _scan: { value: function (pattern, atPos, asStr, advance) {
            ...
            src = this.source;
            result = pattern.exec(src.slice(pos), atPos?false:true);
            if (result && (!atPos || (atPos && result.index === 0))) {
                ...
                if (advance) {
                    if(this.last){
                        if(this._before) this._before.push(this.last)
                        else this._before = [this.last];
                     }
