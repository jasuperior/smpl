                ...
                if(!v.grouped){
                    ...
                    if(m){
                        for( var k  in  self.variables ){
                            if(self.variables[k].oindex === i){
                                result = result.replace(k, self[i].expression? (typeof self[i].expression== "function"? self[i].expression.bind(tail):self[i].expression): m[0]); //may need to extend this to account for patterns that have function expressions
                                // captured_vars[k] = m[0];
                                // debugger;
                                // console.log(result)
                            }
                    ...
                    if(v.delim) {
                        ...
                        if(prematch){
                            ...
                            prematch = prematch.map(function(v){ return v.trim().replace(this.delim, "")}.bind(v));
                            ...
                            keys.forEach((variable)=>{
                                ...
                                // console.log("this is your pattern ::: ", variable, pattern);
