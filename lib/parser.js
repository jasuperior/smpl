...
var Pattern = (function(){
    ...
    function tokenize ( value ) {
        ...
        var types = Object.keys(tokens), pattern = [], index = 0, groupcount = 0, skip;
        for(var i = 0; i < types.length; i++ ){
            ...
            if(result){
                ...
                if(key == "repeat"){
                    ...
                    if( (input.token == "tab"||input.token == "space"||input.token == "newline") && last.type == "whitespace" ) {
                        pattern.pop();
                        skip = true;
                    }
            ...
            if(token.type == "vartype"){
                ...
                if(input = tokens[token.token] || compiler.getPattern(token.token) ){
                    ...
                    if(token.repeat){
                        ...
                        if(!(token.token == "tab"||token.token == "space"||token.token == "newline"))
                ...
                if((token.token == "tab"||token.token == "space"||token.token == "newline")) printout = true;
...
var Compiler = (function () {
    ...
    Compiler.prototype.compile = function compile ( str, debug ) {
        ...
        str = new Source(this.input || str), plen = this.patterns.length, _plen = plen;
