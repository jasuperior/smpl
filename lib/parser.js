...
Compiler.prototype.compile = function compile ( input, important ) {
    ...
    while(check.length && !program.hasTerminated()){
        ...
        if(reg){
            ...
            // console.log("RESULTS AND ", result.value,"::::::", toReplace, "::::: INPUT :::::::::", input );
            ...
            reg.rule.forEach(function(v){
                if(v.expression){
                    ...
                    var newReplace = toReplace.replace(global_reg, typeof v.expression == "function"?v.expression.bind(input):v.expression);
