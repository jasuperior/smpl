        ...
        for(var i = 0; i < types.length; i++ ){
            ...
            if(result){
                ...
                if(key == "repeat"){
                    ...
                    var last =  pattern[pattern.length-1];
                    if( last.type == "repeat" && last.alt  == input.value){
...
//used to
    ...
    Pattern.prototype.getPatternString = function getPatternString ( context, start ) {
        ...
        var parts = this.tokens.map((token)=>{
            ...
            if(token.type == "vartype"){
                ...
                if( token.var ){
                    result += `(?<${token.var}>(${token.reg_pattern.source}))`;
    ...
    Pattern.prototype.check = function check ( str ) {
        var result = this.getPattern().exec(str);
        if(result){
            return result.index;
        }
    }
...
var returnPattern = (function(){
         ...
         if(pattern.vars){//TODO: Must make pattern have a var property to patterns, so that i can use the keys to look up the props in obj;
             ...
             values.forEach((value)=>{
                 ...
                 pattern.vars.forEach(function(v){
                     ...
                     if(value.T && !var_obj[v].T ) var_obj[v].transformPattern = var_obj[v].T = function transform ( delim ) {
                         if(this.map)
                         return this.map((str , i)=>{
                             if(str.transformPattern) return str.transformPattern();
                             return str;
    ...
    returnPattern.prototype.toFunction = function toFunction ( input, compiler ){
        return new Function("",input).bind(compiler||this._compiler);
    }
    ...
    Compiler.prototype.compile = function compile ( str ) {
        ...
        while(!done){
            ...
            if(pat){
                ...
                // console.log(str.computedCursor)
            }else
                done = true;
...
var c = new Compiler();
c.addPattern("$age:lit!", function(){ return `nother(${this.age})`}, { name: "other" })
c.addPattern("$thing:other!", null, { name: "person" })
c.addPattern("little $person:person", null, { name: "example" })
var a = c.addPattern("hello $place:example(,)...", function(){
    // console.log(this.G[0].place);
    return this.place.person.thing.map((v)=>{
        // console.log(this)
        this.addCapture(v)
        return v.T();
    }).join(",")
}, { name: "something", capture: true }), b;
c.addPattern("welcome $other:$something", function(){
    // console.log("we in here", this.other);
    return `play_with(${ this.other })`
})
c.addPattern("cheerio {...}",null,{ name:"ellip"})
c.addPattern("pipip $var... bagel",function(){
    return `pipped(${ this.var })`;
},{ name:"pip"})
c.addPattern("hiho {$nobody...}",function(){
    console.log(this.toFunction(this.nobody)())
    return this.nobody;
},{ name:"hiho"})
// console.log(a)
b = a.getPattern()
...
console.log(c.compile(`
    hiho { return 2 }
    `) );
// console.log(c.getPattern("hiho").getPatternString())
